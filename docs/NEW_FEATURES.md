...
### ✅ B4. Leg 2 Phase C — migration guard — **shipped**
`content-flow.migrated` localStorage key is the one-way door marker. `markMigrated()` fires at the top of `hydrateFromBridge` before any IDB-destructive operation, so a mid-hydration crash still leaves the device correctly flagged. `seedIfEmpty` bails when either `isBridgeMode()` OR `isMigrated()` is true — that second guard prevents a user who signs out of Opsette and returns to the standalone URL from seeing demo content reseeded on top of their (now-cleared) IDB. Helpers `isMigrated` / `markMigrated` in `src/lib/device.ts`. No user-facing UI.

### ✅ B5. Data-loss banner for standalone — **shipped**
`src/components/DataLossBanner.tsx` mounted in `AppLayout` above the breadcrumb slot. antd `Alert` (info, closable), title "Your content lives in this browser", description explains the device-local risk, two buttons: "Export backup" (navigates to `/settings` via `useNavigate`) and "Sync with Opsette" (opens `https://opsette.io` in a new tab). Dismissal persists via `content-flow.banner-dismissed.v1` localStorage (sticky across reloads, survives mode switches). Returns null in bridge mode — never shown inside the Opsette iframe regardless of dismissal state. Helpers `isBannerDismissed` / `dismissBanner` in `src/lib/device.ts`.
